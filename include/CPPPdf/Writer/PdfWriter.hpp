...
struct PdfTilingPatternOptions final {
    ...
    // Legacy source-compatibility flags retained from the original API.
    // Pattern direction is controlled by the content stream and matrix.
    bool horizontal{false};
    bool vertical{false};
};

enum class PdfDeviceColorSpace { Gray, Rgb, Cmyk };

struct PdfIccColorSpaceOptions final {
    std::string name; // page resource name, e.g. "CS1"
    std::vector<std::byte> profileBytes;
    std::uint8_t components{3U};
    PdfDeviceColorSpace alternate{PdfDeviceColorSpace::Rgb};
};

struct PdfSeparationColorSpaceOptions final {
    std::string name; // page resource name
    std::string colorantName;
    PdfDeviceColorSpace alternate{PdfDeviceColorSpace::Cmyk};
    std::vector<double> c0{0.0, 0.0, 0.0, 0.0};
    std::vector<double> c1{0.0, 0.0, 0.0, 1.0};
    double exponent{1.0};
};

struct PdfDeviceNColorSpaceOptions final {
    std::string name; // page resource name
    std::vector<std::string> colorantNames;
    PdfDeviceColorSpace alternate{PdfDeviceColorSpace::Cmyk};
    // PostScript calculator-function body. Inputs are the DeviceN tint values;
    // the program must leave one component per alternate color channel.
    std::string tintTransformProgram;
};


struct PdfMeshVertex final {
    PdfPoint position{};
    std::vector<double> colorComponents;
};

// ShadingType 4: independent Gouraud-shaded triangles. Pdf++ encodes the mesh
// with 16-bit coordinates, 8-bit components and 2-bit vertex flags.
struct PdfFreeFormMeshShadingOptions final {
    std::string name;
    PdfDeviceColorSpace colorSpace{PdfDeviceColorSpace::Rgb};
    std::vector<std::array<PdfMeshVertex, 3>> triangles;
    bool antiAlias{true};
};

// ShadingType 5: a regular lattice of vertices, stored row-major.
struct PdfLatticeMeshShadingOptions final {
    std::string name;
    PdfDeviceColorSpace colorSpace{PdfDeviceColorSpace::Rgb};
...
// Four corner colors are ordered as the four patch corners associated with the
// control-point order defined by PDF 32000 for patch mesh shadings.
using PdfPatchCornerColors = std::array<std::vector<double>, 4>;

// ShadingType 6: independent Coons patches. This initial high-level API emits
// flag-0 patches, which are fully self-contained and interoperable with PDF
// viewers; shared-edge flag compression may be added transparently later.
struct PdfCoonsPatch final {
    std::array<PdfPoint, 12> controlPoints{};
    PdfPatchCornerColors cornerColors{};
};

struct PdfCoonsPatchMeshShadingOptions final {
    std::string name;
    PdfDeviceColorSpace colorSpace{PdfDeviceColorSpace::Rgb};
    std::vector<PdfCoonsPatch> patches;
    bool antiAlias{true};
};

// ShadingType 7: independent tensor-product patches with a 4x4 Bezier control
// grid. Patches are emitted with flag 0 so no edge-reuse state is required.
struct PdfTensorProductPatch final {
    std::array<PdfPoint, 16> controlPoints{};
    PdfPatchCornerColors cornerColors{};
};

struct PdfTensorProductPatchMeshShadingOptions final {
    std::string name;
    PdfDeviceColorSpace colorSpace{PdfDeviceColorSpace::Rgb};
    std::vector<PdfTensorProductPatch> patches;
    bool antiAlias{true};
};
...
struct PdfLinkOptions final {
    ...
    double borderWidth{1.0};
    // Required for PDF/UA. Used for /Contents and the tagged /Link OBJR.
    std::string accessibleDescription;
    std::string structureRole{"Link"};
};
...
struct PdfFileAttachmentOptions final {
    ...
    std::string structureRole{"Annot"};
};
...
public:
    ...
    // Registers named writer color spaces usable through PdfCanvas::SetFillColorSpace
    // and SetStrokeColorSpace.
    [[nodiscard]] std::size_t AddIccColorSpace(const PdfIccColorSpaceOptions& options);
    [[nodiscard]] std::size_t AddSeparationColorSpace(const PdfSeparationColorSpaceOptions& options);
    [[nodiscard]] std::size_t AddDeviceNColorSpace(const PdfDeviceNColorSpaceOptions& options);
    [[nodiscard]] std::size_t AddFreeFormMeshShading(const PdfFreeFormMeshShadingOptions& options);
    [[nodiscard]] std::size_t AddLatticeMeshShading(const PdfLatticeMeshShadingOptions& options);
    [[nodiscard]] std::size_t AddCoonsPatchMeshShading(const PdfCoonsPatchMeshShadingOptions& options);
    [[nodiscard]] std::size_t AddTensorProductPatchMeshShading(
        const PdfTensorProductPatchMeshShadingOptions& options);
    void ClearColorSpaces();
    [[nodiscard]] std::size_t GetColorSpaceCount() const noexcept;
    [[nodiscard]] std::size_t GetMeshShadingCount() const noexcept;
    ...
    // Configures creation-time PDF/A metadata and output intent. The ICC
    // profile is embedded verbatim as /DestOutputProfile. PDF/A-1 forces a
    // PDF 1.4/classic-xref file; PDF/A-2/3 use PDF 1.7; PDF/A-4 uses PDF 2.0.
    void ConfigureForPdfA(PdfConformanceProfile profile,
                          std::span<const std::byte> iccProfile,
                          std::string outputConditionIdentifier = "sRGB IEC61966-2.1");
    void ClearPdfAConfiguration() noexcept;
    [[nodiscard]] std::optional<PdfConformanceProfile> GetPdfAProfile() const noexcept;

    // Configures PDF/UA-1 (PDF 1.7) or PDF/UA-2 (PDF 2.0). Save fails
    // closed if tagged-content, metadata, language, title, annotation or font
    // requirements that can be checked by the writer are violated.
    void ConfigureForPdfUa(PdfConformanceProfile profile,
                           std::string language,
                           std::string title);
    void ClearPdfUaConfiguration() noexcept;
    [[nodiscard]] std::optional<PdfConformanceProfile> GetPdfUaProfile() const noexcept;
    void SetConformanceEnforcement(bool enabled = true) noexcept;
    [[nodiscard]] bool GetConformanceEnforcement() const noexcept;
