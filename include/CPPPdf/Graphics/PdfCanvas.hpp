...
enum class PdfPatternBaseColorSpace { Gray, Rgb, Cmyk };
...
enum class PdfStructurePlacement { Block, Inline, Before, Start, End };
enum class PdfStructureTextAlignment { Start, Center, End, Justify };
enum class PdfTableScope { None, Row, Column, Both };

// Standard structure attributes used by PDF/UA assistive technology. Layout
// attributes are written with owner /Layout; table attributes use owner /Table.
struct PdfStructureAttributes final {
    std::optional<PdfStructurePlacement> placement;
    std::optional<PdfStructureTextAlignment> textAlignment;
    std::optional<PdfRectangle> boundingBox;
    std::optional<double> width;
    std::optional<double> height;
    std::uint32_t rowSpan{1U};
    std::uint32_t columnSpan{1U};
    PdfTableScope scope{PdfTableScope::None};
    std::vector<std::string> headers;
};

struct PdfMarkedContentOptions final {
...
public:
    ...
    PdfCanvas& SetStrokeColorSpace(std::string colorSpaceName,
                                   std::span<const double> components);
    PdfCanvas& SetFillColorSpace(std::string colorSpaceName,
                                 std::span<const double> components);
    PdfCanvas& SetStrokeOpacity(double opacity);
    PdfCanvas& SetFillOpacity(double opacity);
    // Selects a colored tiling pattern (PaintType 1) registered via
    // PdfWriter::AddTilingPattern.
    PdfCanvas& SetPattern(std::string patternName, bool applyToFill = true, bool applyToStroke = true);
    // Selects an uncolored tiling pattern (PaintType 2) together with its
    // underlying device color space and tint components.
    PdfCanvas& SetUncoloredPattern(std::string patternName,
                                   PdfPatternBaseColorSpace baseColorSpace,
                                   std::span<const double> components,
                                   bool applyToFill = true,
                                   bool applyToStroke = true);
    PdfCanvas& SetOpacity(double opacity);
    PdfCanvas& SetBlendMode(PdfBlendMode mode);
    // Applies stroke/fill alpha and a blend mode as one ExtGState. Unlike
    // issuing separate gs operators, this preserves all three values together.
    PdfCanvas& SetTransparency(double strokeOpacity, double fillOpacity,
                               PdfBlendMode mode = PdfBlendMode::SourceOver);
    ...
    // Uses an Identity-V Type0 font with /DW2 and /W2 vertical metrics.
    PdfCanvas& SetTrueTypeFontAndSizeVertical(const PdfTrueTypeFont& font, double size);
    ...
    // Selects a user-defined Type3 font. Glyph programs are embedded as CharProcs.
    PdfCanvas& SetType3FontAndSize(const PdfType3Font& font, double size);
    // Shows already encoded one-byte Type3 character codes.
    PdfCanvas& ShowType3Text(std::string encodedBytes);
    // Maps UTF-8 code points through PdfType3Glyph::unicodeCodePoint.
    PdfCanvas& ShowType3TextUtf8(std::string utf8Text);
    ...
    PdfCanvas& EndText();
    // Starts a tagged marked-content sequence and assigns the next page-local
    // MCID. A matching structure element is emitted into the document
    // structure tree at save time. The writer is automatically marked tagged.
    PdfCanvas& BeginMarkedContent(std::string role,
                                  std::string alternativeText = {},
                                  std::string actualText = {});
    PdfCanvas& BeginMarkedContent(const PdfMarkedContentOptions& options);
    // Marks purely decorative/pagination content that should be ignored by
    // assistive technology. `artifactType` may be Pagination, Layout, Page or
    // Background; an empty value emits /Artifact BMC.
    PdfCanvas& BeginArtifact(std::string artifactType = {});
    PdfCanvas& EndMarkedContent();

    PdfCanvas& DrawImage(const PdfImage& image, const PdfRectangle& rectangle);
    // Writes a self-contained inline image (BI/ID/EI) into the page content
    // instead of creating an XObject. Intended for small, one-off images.
    // Payload bytes are ASCIIHex wrapped so binary data cannot terminate the
    // inline image accidentally.
    PdfCanvas& DrawInlineImage(const PdfImage& image, const PdfRectangle& rectangle);
    // Paints a named mesh shading registered on PdfWriter using the `sh` operator.
    PdfCanvas& PaintShading(std::string shadingName);
