...
public:
    ...
    struct SignedDataOptions final {
        // Additional issuer certificates, ordered from the leaf issuer toward
        // the trust anchor. The leaf certificate is supplied separately.
        std::vector<std::vector<std::uint8_t>> certificateChain;
        std::string signerName{"Pdf++ Signer"};
        // Zero selects the current UTC time. Set includeSigningTime to false
        // for deterministic signatures or externally managed attributes.
        std::uint64_t signingTimeSeconds{};
        bool includeSigningTime{true};
    };

    struct SignedDataInfo final {
        std::vector<std::vector<std::uint8_t>> certificates;
        std::vector<std::uint8_t> signature;
        // Canonical DER SET OF encoding (tag 0x31) used as the signature input
        // when signed attributes are present.
        std::vector<std::uint8_t> signedAttributesDer;
        std::array<std::uint8_t, 32> messageDigest{};
        std::uint64_t signingTimeSeconds{};
        bool hasSignedAttributes{};
        bool hasMessageDigest{};
        bool hasContentType{};
    };

    // Builds CMS/PKCS#7 SignedData with detached content, DER-sorted signed
    // attributes (content-type, message-digest and optional signing-time), one
    // RSA signer, and an optional embedded certificate chain.
    ...
    // Parses the signer information, embedded certificate chain and signed
    // attributes from a CMS SignedData DER value.
    [[nodiscard]] static bool ParseSignedData(
        std::span<const std::uint8_t> signedData,
        SignedDataInfo& outInfo);

    // Backward-compatible extraction of the first certificate and signature.
    [[nodiscard]] static bool ParseSignedData(
        std::span<const std::uint8_t> signedData,
