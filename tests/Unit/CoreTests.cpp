...
int RunCoreTests() {
    ...
    PdfContentProcessor dashProcessor;
    int dashEvents = 0;
    std::vector<double> parsedDashPattern;
    double parsedDashPhase = 0.0;
    dashProcessor.SetHandler([&](const PdfContentEvent& event) {
    ...
    dashProcessor.Process("[3 2] 1.5 d 5 w 0 0 m 100 0 l S");
    PDFPP_TEST_CHECK(dashEvents == 1);
    PDFPP_TEST_CHECK(parsedDashPattern.size() == 2U);
    PDFPP_TEST_CHECK(std::abs(parsedDashPattern[0] - 3.0) < 1.0e-9);
    PDFPP_TEST_CHECK(std::abs(parsedDashPattern[1] - 2.0) < 1.0e-9);
    PDFPP_TEST_CHECK(std::abs(parsedDashPhase - 1.5) < 1.0e-9);
...
int RunPdfAValidationTests() {
    using namespace CPPPdf;
    // A conforming PDF/A-1b file passes all hard checks.
    const auto conforming = buildConformancePdf(true, true);
    {
        ...
        for (const auto& issue : result.issues) {
            if (issue.error && issue.code == "PDFA-METADATA-001") metadataError = true;
            if (issue.error && issue.code == "PDFA-OUTPUT-001") outputError = true;
            if (issue.error && issue.code == "PDFA-OUTPUT-002") outputError = true;
        }
        PDFPP_TEST_CHECK(!metadataError);
        PDFPP_TEST_CHECK(!outputError);
    }

    // Missing metadata and output intent are both hard failures.
    {
    ...
    // A pdfaid:part mismatch is reported as a hard error.
    {
        ...
        // Replace keeping the same length so stream /Length stays valid.
        ...
        for (const auto& issue : result.issues) {
            if (issue.error && issue.code == "PDFA-METADATA-003") partError = true;
        }
        PDFPP_TEST_CHECK(partError);
    }

    // PDF/UA requires a structure tree root.
    {
        const auto document = PdfDocument::Open(std::span<const std::byte>(conforming));
        const auto result = PdfConformanceValidator::Validate(document, PdfConformanceProfile::PdfUA1);
        bool tagError = false;
        for (const auto& issue : result.issues) {
            if (issue.error && issue.code == "PDFUA-TAG-001") tagError = true;
        }
        PDFPP_TEST_CHECK(tagError);
    }
    return 0;
}
...
int main() {
    ...
    runner.Run("Validation.PdfA", RunPdfAValidationTests);
