...
void TestTextSearch() {
    ...
    PdfTextSearchOptions limited;
    limited.maxMatches = 1;
    PDFPP_TEST_CHECK(PdfTextSearch::Find(chunks, "o", limited).size() == 1);
    ...
    // Whitespace handling.
    PDFPP_TEST_CHECK(PdfTextLayout::IsWhitespace(" \t\n"));
    PDFPP_TEST_CHECK(!PdfTextLayout::IsWhitespace(" a"));
    PDFPP_TEST_CHECK(PdfTextLayout::TrimWhitespace("  hi \u00a0") == "hi");
    // Line splitting keeps empty lines.
    ...
    PDFPP_TEST_CHECK(PdfTextLayout::TitleCase("hELLO wORLD") == "Hello World");
...
void TestPageEditingAndOrganization(const std::filesystem::path& base) {
    ...
    const auto moved = TempPath("pdfpp_api_moved.pdf");
    const auto moveResult = PdfPageOrganizer::MovePage(base, moved, 0U, 1U);
    PDFPP_TEST_CHECK(moveResult.outputPageCount == 2);
    PDFPP_TEST_CHECK(PdfDocument::Open(moved).GetPageText(0).find("page two") != std::string::npos);
    ExpectThrows([&] { (void)PdfPageOrganizer::MovePage(base, moved, 9U, 0U); });
...
void TestPublicApiArchitecture() {
    ...
    static_assert(CPPPdf::VersionMinor == 153U);
    ...
    PDFPP_TEST_CHECK(CPPPdf::VersionString == "0.153.0");
}
