...
- The renderer caches decoded images per object number within a render pass and supersamples via `Downsample` when `antiAliasSamples > 1`. The writer preserves JPEG/JPX/CCITT encodings (`preserveImageEncodings`), `PdfImage::FromJpeg2000`/`FromCcitt` create native payloads, `EncodeCcittG4` encodes 1-bit images, and `EncodeJpeg` writes baseline JPEGs from RGB.
...
- `PdfType1Font` parses PFB/PFA programs and the writer embeds `/Subtype /Type1` fonts with `/FontFile`, `/WinAnsiEncoding`, and `/Widths`; `SetEmbeddedCffFontAndSize` embeds CFF fonts as `/FontFile3 /Type1C`.
...
- `CertificateInfoOf` extracts subject/issuer common names, validity window, and self-signed flag from a DER X.509 certificate; `ValidateCertificate` checks expiry and a basic issuer chain.
