...
PdfBitmap::PdfBitmap(const std::size_t width, const std::size_t height, const PdfRgbaColor background)
    : width_(width), height_(height), pixels_(width * height * 4U) {
    Clear(background);
}
...
void PdfBitmap::BlendPixel(const std::int32_t x, const std::int32_t y, const PdfRgbaColor color) {
    ...
    const auto blend = [alpha, inverse](const std::uint8_t source, const std::uint8_t destination) {
        return static_cast<std::uint8_t>((source * alpha + destination * inverse + 127U) / 255U);
    };
    ...
    pixels_[offset + 3U] = static_cast<std::byte>(255U);
}
