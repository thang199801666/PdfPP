...
std::array<std::uint8_t,16> ownerKey(std::string_view password){auto digest=md5(padPassword(password));for(int i=0;i<50;++i)digest=md5(digest);return digest;}
std::array<std::uint8_t,16> fileKeyFromPadded(std::span<const std::uint8_t,32> padded,
    std::span<const std::uint8_t,32> owner,std::int32_t permissions,std::span<const std::uint8_t> id,bool encryptMetadata){
    std::vector<std::uint8_t>d;d.reserve(32+32+4+id.size()+4);d.insert(d.end(),padded.begin(),padded.end());d.insert(d.end(),owner.begin(),owner.end());const auto p=static_cast<std::uint32_t>(permissions);for(unsigned i=0;i<4;++i)d.push_back(static_cast<std::uint8_t>(p>>(8U*i)));d.insert(d.end(),id.begin(),id.end());if(!encryptMetadata)d.insert(d.end(),4,0xffU);auto digest=md5(d);for(int i=0;i<50;++i)digest=md5(digest);return digest;
}
std::array<std::uint8_t,32> computeUser(std::span<const std::uint8_t,16> key,std::span<const std::uint8_t> id){std::vector<std::uint8_t>d(PasswordPadding.begin(),PasswordPadding.end());d.insert(d.end(),id.begin(),id.end());auto digest=md5(d);auto encrypted=rc4(digest,key);std::array<std::uint8_t,16>pass{};for(std::uint8_t i=1;i<=19;++i){for(std::size_t j=0;j<16;++j)pass[j]=key[j]^i;encrypted=rc4(encrypted,pass);}std::array<std::uint8_t,32>u{};std::copy(encrypted.begin(),encrypted.end(),u.begin());return u;}
bool matchesUser(std::span<const std::uint8_t,16> key,std::span<const std::uint8_t,32> expected,std::span<const std::uint8_t> id){const auto u=computeUser(key,id);return std::equal(u.begin(),u.begin()+16,expected.begin());}
...
PdfStandardSecurity PdfStandardSecurity::Create(const PdfEncryptionOptions&o,std::span<const std::uint8_t,16>id){PdfStandardSecurity s;s.algorithm_=o.algorithm;s.permissions_=BuildPdfPermissionBits(o.permissions);s.encryptMetadata_=o.encryptMetadata;std::copy(id.begin(),id.end(),s.fileId_.begin());const auto ownerPassword=o.ownerPassword.empty()?o.userPassword:o.ownerPassword;const auto key=ownerKey(ownerPassword);auto owner=rc4(padPassword(o.userPassword),key);std::array<std::uint8_t,16>pass{};for(std::uint8_t i=1;i<=19;++i){for(std::size_t j=0;j<16;++j)pass[j]=key[j]^i;owner=rc4(owner,pass);}std::copy(owner.begin(),owner.end(),s.ownerEntry_.begin());const auto padded=padPassword(o.userPassword);s.fileKey_=fileKeyFromPadded(padded,s.ownerEntry_,s.permissions_,s.fileId_,s.encryptMetadata_);s.userEntry_=computeUser(s.fileKey_,s.fileId_);s.authenticated_=true;s.ownerAuthenticated_=true;return s;}

PdfStandardSecurity PdfStandardSecurity::Parse(std::string_view d,std::span<const std::uint8_t>id){PdfStandardSecurity s;const auto filter=regexValue(d,R"(/Filter\s*/([^\s/<>()\[\]]+))");if(filter!="Standard")throw PdfException(PdfErrorCode::UnsupportedFeature,"Only the PDF Standard Security Handler is supported.");const int r=std::stoi(regexValue(d,R"(/R\s+(-?\d+))"));const int v=std::stoi(regexValue(d,R"(/V\s+(-?\d+))"));if(r==4&&v==4){if(d.find("/CFM /AESV2")==std::string_view::npos)throw PdfException(PdfErrorCode::UnsupportedEncryption,"Only AESV2 crypt filters are supported for revision 4 encryption.");s.algorithm_=PdfEncryptionAlgorithm::Aes128;}else if(r==3&&(v==2||v==3))s.algorithm_=PdfEncryptionAlgorithm::Rc4_128;else throw PdfException(PdfErrorCode::UnsupportedEncryption,"Unsupported PDF encryption revision. Supported: AES-128 (R4) and RC4-128 (R3).");s.permissions_=static_cast<std::int32_t>(std::stoll(regexValue(d,R"(/P\s+(-?\d+))")));s.encryptMetadata_=d.find("/EncryptMetadata false")==std::string_view::npos;auto o=ParsePdfHex(regexValue(d,R"(/O\s*<([0-9A-Fa-f\s]+)>)"));auto u=ParsePdfHex(regexValue(d,R"(/U\s*<([0-9A-Fa-f\s]+)>)"));if(o.size()!=32||u.size()!=32||id.size()<16)throw PdfException(PdfErrorCode::UnsupportedEncryption,"Malformed encryption owner/user key or file ID.");std::copy_n(o.begin(),32,s.ownerEntry_.begin());std::copy_n(u.begin(),32,s.userEntry_.begin());std::copy_n(id.begin(),16,s.fileId_.begin());return s;}

bool PdfStandardSecurity::Authenticate(std::string_view password){const auto padded=padPassword(password);auto candidate=fileKeyFromPadded(padded,ownerEntry_,permissions_,fileId_,encryptMetadata_);if(matchesUser(candidate,userEntry_,fileId_)){fileKey_=candidate;authenticated_=true;ownerAuthenticated_=false;return true;}const auto key=ownerKey(password);std::vector<std::uint8_t>recovered(ownerEntry_.begin(),ownerEntry_.end());std::array<std::uint8_t,16>pass{};for(int i=19;i>=0;--i){for(std::size_t j=0;j<16;++j)pass[j]=key[j]^static_cast<std::uint8_t>(i);recovered=rc4(recovered,pass);}std::array<std::uint8_t,32>ownerRecovered{};std::copy_n(recovered.begin(),32,ownerRecovered.begin());candidate=fileKeyFromPadded(ownerRecovered,ownerEntry_,permissions_,fileId_,encryptMetadata_);if(matchesUser(candidate,userEntry_,fileId_)){fileKey_=candidate;authenticated_=true;ownerAuthenticated_=true;return true;}return false;}

std::string PdfStandardSecurity::EncryptionDictionary()const{std::string d="<< /Filter /Standard ";if(algorithm_==PdfEncryptionAlgorithm::Aes128)d+="/V 4 /R 4 /Length 128 /CF << /StdCF << /CFM /AESV2 /AuthEvent /DocOpen /Length 16 >> >> /StmF /StdCF /StrF /StdCF ";else d+="/V 2 /R 3 /Length 128 ";d+="/O <"+PdfHex(ownerEntry_)+"> /U <"+PdfHex(userEntry_)+"> /P "+std::to_string(permissions_);if(!encryptMetadata_)d+=" /EncryptMetadata false";d+=" >>";return d;}

std::vector<std::uint8_t> PdfStandardSecurity::Crypt(std::span<const std::uint8_t>input,std::uint32_t objectNumber,std::uint16_t generation,bool encrypt)const{if(!authenticated_)throw PdfException(PdfErrorCode::PasswordRequired,"A valid password is required to access encrypted PDF objects.");std::vector<std::uint8_t>material(fileKey_.begin(),fileKey_.end());material.push_back(static_cast<std::uint8_t>(objectNumber));material.push_back(static_cast<std::uint8_t>(objectNumber>>8U));material.push_back(static_cast<std::uint8_t>(objectNumber>>16U));material.push_back(static_cast<std::uint8_t>(generation));material.push_back(static_cast<std::uint8_t>(generation>>8U));if(algorithm_==PdfEncryptionAlgorithm::Aes128){material.insert(material.end(),{'s','A','l','T'});}const auto digest=md5(material);if(algorithm_==PdfEncryptionAlgorithm::Rc4_128)return rc4(input,digest);return aesCbc(input,digest,encrypt);}
